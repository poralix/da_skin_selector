...
$admin_tabs = array("skins","settings","logs","about");
$admin_tabs_content = array();

    $_selected_tab=0;
    do_output("<div id='px__skin_selector'><h1>".$px_Skin_Selector->get_lang('ADMIN_CONFIGURE_PLUGIN').":</h1>");
    do_output("<div id='px__tabs' style='display:none;'>\n<ul>");
    foreach ($admin_tabs as $tab)
    {
        $admin_tabs_content[$tab] = "";
        do_output("<li><a href='#".$tab."'>".$px_Skin_Selector->get_lang('ADMIN_TAB_'.strtoupper($tab))."</a></li>");
    }
    do_output("</ul>");
    foreach ($admin_tabs_content as $tab => $content)
    {
        if (is_file(PLUGIN_ADMIN_TABS_DIR."/".$tab.".php")) {
            require_once(PLUGIN_ADMIN_TABS_DIR."/".$tab.".php");
        }
        do_output("<div id='".$tab."' class='skin_tab_admin'>".$content."</div>");
    }
