...
#define PATH_DST "/usr/local/directadmin/plugins/da_skin_selector/images"
#define ALLOWED_MIME_TYPE "image/jpeg"
...
int main (int argc, const char * argv[])
{
    ...
    if (argc < 3){
        printf("usage: %s [old_name.jpg] [new_name]\n", argv[0]);
        return 0;
    }
    ...
    sprintf(p_newname, "%s/%s.jpg", PATH_DST, b_newname);
    ...
    sprintf(cmd, "file -i %s | grep -c '%s;' > /dev/null", p_oldname, ALLOWED_MIME_TYPE);
    ret = system(cmd);

    if(ret == 0) 
    {
        printf("[OK] File %s seems to be JPEG image!\n",p_oldname);
    }
    else 
    {
